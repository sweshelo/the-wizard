...
イベントは連続して発生するため、以下のタイミングでAPI呼び出しを行う:

| トリガー | 説明 |
|---------|------|
| `Choices` | 選択肢イベント受信時 |
| `Operation(defrost)` | freeze後のdefrost時 |
| `DisplayEffect(Continue)` | 続行確認要求時 |

**処理フロー:**
```
イベント受信 → UIに即座に表示 + バッファに蓄積
            ↓
判断トリガー受信 → バッファ内イベントをまとめてAPIに送信
```

**タイマー不使用:** デバウンスやタイマーは使用せず、サーバーからのメッセージのみをトリガーとする
